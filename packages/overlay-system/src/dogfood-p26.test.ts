...
let initialState: ReturnType<typeof getSetupState> | null = null;

beforeAll(() => {
  initialState = getSetupState(TARGET_DIR);
});
...
describe('Phase 26 Dogfood — First-Run Setup', () => {
  it('DF26-01: detect project from shadcn-admin', () => {
    ...
    if (result.ok) {
      ...
      console.log(`  DF26-01: project="${result.value.name}", framework=${result.value.framework ?? 'unknown'}, pm=${result.value.packageManager ?? 'unknown'}`);
    }
  });
  ...
  it('DF26-02: initialize workspace', () => {
    ...
    if (result.ok) {
      ...
      console.log(`  DF26-02: workspace initialized with ${result.value.directories.length} directories`);
    }
  ...
  it('DF26-04: MCP runtime tools/list verification', async () => {
    ...
    console.log(`  DF26-04: MCP static scan — ${staticVerification.toolsFound.length} tools found via entry.ts`);
    ...
    try {
      const runtimeResult = await Promise.race([
        ...
        new Promise<never>((_, reject) => setTimeout(() => reject(new Error('MCP server startup timeout')), 30000)),
      ]);
      ...
      if (runtimeResult.ok) {
        ...
        console.log(`  DF26-04: MCP runtime tools/list — ${runtimeResult.value.toolsFound.length} tools confirmed via actual MCP server`);
      }
    } catch (e) {
      ...
      console.log(`  DF26-04: MCP runtime verification timed out (${(e as Error).message}) — static verification passed as primary gate`);
      console.log(`  DF26-04: Note: MCP server startup takes 20+ seconds due to eager package imports. This is a pre-existing performance issue.`);
    }
  }, 60000);
  ...
  it('DF26-05: run setup checks with browser verification', async () => {
    ...
    console.log(`  DF26-05: checks=${checks.length} (required=${required.length}, recommended=${recommended.length}, optional=${optional.length})`);

    for (const check of checks) {
      const mark = check.status === 'pass' ? '✅' : check.status === 'warning' ? '⚠️' : check.status === 'fail' ? '❌' : '⏭️';
    ...
    const criticalFailures = required.filter((c) =>
      c.status === 'fail' &&
  ...
  it('DF26-07: capture smoke proves packet creation via VCE generatePacket', async () => {
    ...
    if (result.ok) {
      ...
      console.log(`  DF26-07: packetId=${result.value.packetId} (opaque UUID from VCE generatePacket)`);
  ...
  it('DF26-08: complete setup', async () => {
    ...
    if (result.ok) {
      ...
      console.log(`    capabilities: captureContext=${result.value.capabilities.captureContext}, browserRuntime=${result.value.capabilities.browserRuntime}`);
    }
  }, 180000);
  ...
  it('DF26-13: real Phase 21-25 readiness', async () => {
    ...
    // Source hints
    const hintsCheck = checks.find((c) => c.checkId === 'source-hints' || c.checkId === 'usage-site-hints');
    if (hintsCheck) {
  ...
  it('DF26-15: app reachability check — unreachable app produces required failure', async () => {
    const checks = await runAllChecks({ projectRoot: TARGET_DIR, appUrl: 'http://localhost:99999' });
  ...
  it('DF26-17: agent config readiness', () => {
    ...
    console.log(`  DF26-17: agent config — detected=${config.detected}, kind=${config.kind}, verified=${config.verified}`);
  ...
  it('DF26-18: completeSetup persists appUrl', async () => {
    ...
    const checks = await runAllChecks({ projectRoot: TARGET_DIR, limitedMode: true } as any);
    const result = completeSetup({
      projectRoot: TARGET_DIR,
      project: project.value,
      checks,
  ...
  it('DF26-19: real first capture smoke against local app URL', async () => {
    ...
    const result = await runSmoke({ projectRoot: TARGET_DIR, url: 'data:text/html,<html><body><h1>Smoke Test</h1></body></html>' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.packetId).toBeTruthy();
