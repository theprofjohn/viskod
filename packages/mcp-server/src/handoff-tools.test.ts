...
describe('MCP list_agent_handoffs', () => {
  it('lists handoffs in deterministic order', async () => {
    ...
    await handoffService.createHandoff({ issueId: issue1.value.issueId }, 'mcp-session', 'mcp-page');
    await new Promise(r => setTimeout(r, 10));
    await handoffService.createHandoff({ issueId: issue2.value.issueId }, 'mcp-session', 'mcp-page');
...
describe('MCP output redaction', () => {
  it('no unredacted secrets in get_agent_handoff output', async () => {
    ...
    const issue = await issueService.createIssue(selection, TEST_SESSION_ID, TEST_PAGE_ID, 'Secret test');
    expect(issue.ok).toBe(true);
    if (!issue.ok) return;
  ...
  it('no email in get_agent_handoff output', async () => {
    ...
    const issue = await issueService.createIssue(selection, TEST_SESSION_ID, TEST_PAGE_ID, 'Email test');
    expect(issue.ok).toBe(true);
    if (!issue.ok) return;
