...
import { filterCandidates, scoreCandidate, scoreAndRank, selectBestCandidate } from './scoring';
...
function target(tag: string, overrides: Partial<VisualSelectionTarget> = {}): VisualSelectionTarget {
  return {
...
describe('DOM Selection Fixtures', () => {
  ...
  it('prefers button over nested span', () => {
    const elements = [
      el('span', { viewportRect: { x: 10, y: 10, width: 80, height: 20 }, isInteractive: false, ancestorDepth: 3, parentTagName: 'button', parentBoundingRect: { x: 10, y: 10, width: 80, height: 40 } }),
      el('button', { viewportRect: { x: 10, y: 10, width: 80, height: 40 }, isInteractive: true, role: 'button', hasVisibleText: true, ancestorDepth: 2, childCount: 2 }),
    ];
  ...
  it('selects icon-only button with accessible label', () => {
    const elements = [
      el('button', { viewportRect: { x: 10, y: 10, width: 40, height: 40 }, isInteractive: true, role: 'button', accessibleName: 'Close', hasVisibleText: false, hasStableAttributes: true }),
      el('i', { viewportRect: { x: 15, y: 15, width: 30, height: 30 }, isInteractive: false, hasVisibleText: false, ancestorDepth: 3, parentBoundingRect: { x: 10, y: 10, width: 40, height: 40 } }),
    ];
  ...
  it('scores label-associated input higher', () => {
    const elements = [
      el('input', { viewportRect: { x: 100, y: 10, width: 200, height: 30 }, isInteractive: true, role: 'textbox', isLabelControl: true, hasStableAttributes: true }),
      el('label', { viewportRect: { x: 10, y: 10, width: 80, height: 30 }, isInteractive: false, hasVisibleText: true, ancestorDepth: 2 }),
    ];
  ...
  it('resolves correctly with duplicate text buttons', () => {
    const elements = [
      el('button', { viewportRect: { x: 10, y: 10, width: 80, height: 30 }, isInteractive: true, role: 'button', accessibleName: 'Save', hasStableAttributes: true, ancestorDepth: 2 }),
      el('button', { viewportRect: { x: 10, y: 50, width: 80, height: 30 }, isInteractive: true, role: 'button', accessibleName: 'Save', hasStableAttributes: false, ancestorDepth: 2 }),
    ];
    const ranked = scoreAndRank(elements, 50, 25);
    expect(ranked.length).toBe(2);
  ...
  it('penalizes extremely deep elements', () => {
    const shallow = el('button', { ancestorDepth: 2, viewportRect: { x: 10, y: 10, width: 100, height: 40 } });
    const deep = el('span', { ancestorDepth: 8, viewportRect: { x: 10, y: 10, width: 100, height: 40 } });
  ...
  it('selects the element under pointer in flex layout', () => {
    const elements = [
      el('button', { viewportRect: { x: 10, y: 10, width: 80, height: 30 }, isInteractive: true, role: 'button', accessibleName: 'Item 1', hasStableAttributes: true }),
      el('button', { viewportRect: { x: 100, y: 10, width: 80, height: 30 }, isInteractive: true, role: 'button', accessibleName: 'Item 2', hasStableAttributes: true }),
      el('button', { viewportRect: { x: 190, y: 10, width: 80, height: 30 }, isInteractive: true, role: 'button', accessibleName: 'Item 3', hasStableAttributes: true }),
    ];
  ...
  it('scores pointer-events-none elements lower', () => {
    const interactive = scoreCandidate(el('button', { isInteractive: true, role: 'button', hasStableAttributes: true }), 50, 20);
    const none = scoreCandidate(el('div', { isInteractive: false, viewportRect: { x: 0, y: 0, width: 100, height: 40 } }), 50, 20);
  ...
  it('box selection finds siblings in a region', () => {
    const candidates = [
      boxEl('a', 'button', { boundingRect: { x: 10, y: 10, width: 80, height: 30 }, isInteractive: true, documentOrder: 1 }),
      boxEl('b', 'button', { boundingRect: { x: 100, y: 10, width: 80, height: 30 }, isInteractive: true, documentOrder: 2 }),
      boxEl('c', 'button', { boundingRect: { x: 190, y: 10, width: 80, height: 30 }, isInteractive: true, documentOrder: 3 }),
    ];
  ...
  it('box selection identifies card region elements', () => {
    const candidates = [
      boxEl('card', 'article', { boundingRect: { x: 10, y: 10, width: 200, height: 300 }, ancestorDepth: 1, isInteractive: false, isTechnical: false }),
      boxEl('title', 'h2', { boundingRect: { x: 20, y: 20, width: 180, height: 30 }, ancestorDepth: 2, isInteractive: false, documentOrder: 1 }),
      boxEl('desc', 'p', { boundingRect: { x: 20, y: 60, width: 180, height: 60 }, ancestorDepth: 2, isInteractive: false, documentOrder: 2 }),
    ];
  ...
  it('box selection excludes overlay nodes', () => {
    const candidates = [
      ...
      boxEl('real', 'button', { boundingRect: { x: 10, y: 10, width: 100, height: 40 }, isInteractive: true }),
    ];
  ...
  it('does not crash on SVG-like bounding rects', () => {
    const svgChild = el('path', { viewportRect: { x: 10, y: 10, width: 5, height: 5 }, isInteractive: false, tagName: 'path' });
  ...
  it('hover mode ignores non-overlay elements with high z-index', () => {
    const elements = [
      el('div', { viewportRect: { x: 0, y: 0, width: 1000, height: 1000 }, ancestorDepth: 1, isTechnical: true, isHidden: false }),
      el('button', { viewportRect: { x: 100, y: 100, width: 100, height: 40 }, isInteractive: true, role: 'button', accessibleName: 'Target', ancestorDepth: 3 }),
    ];
  ...
  it('re-resolution works with text change when attrs match', () => {
    const original = target('button', {
      semantics: { tagName: 'button', isInteractive: true, textPreview: 'Submit form', role: 'button' },
      fingerprints: { stableAttributes: { 'data-testid': 'submit-btn', id: 'submit-button' }, ancestorFingerprint: ['div', 'form'] },
    });
  ...
  it('returns missing after route transition', () => {
    const original = target('button', {
      ...
      fingerprints: { stableAttributes: { 'data-testid': 'dash-link' }, ancestorFingerprint: ['nav', 'header'] },
    });
  ...
  it('resolves after viewport resize when identity matches', () => {
    const original = target('button', {
      semantics: { tagName: 'button', isInteractive: true, textPreview: 'Continue', role: 'button' },
      fingerprints: { stableAttributes: { 'data-testid': 'continue-btn' }, ancestorFingerprint: ['div', 'main'] },
