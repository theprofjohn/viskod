...
| Specification | Architecture Sources | Purpose | Dependencies | Consumers | Priority | Phase | Status | Architecture Risk | Notes |
|--------------|---------------------|---------|-------------|-----------|----------|-------|--------|-------------------|-------|
| `studio.md` | `docs/studio.md`; `docs/architecture.md` §Studio, §State Store; `docs/state-management.md`; `docs/navigation.md`; `docs/ui-architecture.md`; `docs/ARCHITECTURE_BASELINE.md` §Runtime Boundaries | Defines the Studio desktop interface: viewport controls, selection controls, diagnostics display, project status, screenshots, state management, navigation | `shared-types.md`, `error-model.md`, `visual-context-engine.md`, `overlay-system.md` | Developer (human user) | P1 | Application | Draft | Low | Merged `studio-shell.md`, `studio-navigation.md`, `studio-state.md` — one subsystem, one spec |
